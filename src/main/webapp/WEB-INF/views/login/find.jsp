...
<body>
	...
	<main
		...
		<section class="col-5 border rounded-4 mt-5 px-5 py-3">
			<div class="list-group d-flex flex-row justify-content-start"
				id="list-tab">
				<a id="findId" class="active text-decoration-none pe-4 fs-5"
					data-bs-toggle="list" href="#list-login">아이디 찾기</a> <a id="findPw"
					class="text-decoration-none px-4 border-start fs-5"
					data-bs-toggle="list" href="#list-order">비밀번호 찾기</a>
			</div>
			<hr />
					...
					<div class="d-flex flex-column">
						<div class="form-floating">
							<input type="text" class="form-control" id="inputEmail"
								placeholder="name@example.com" /> <label for="floatingInput">가입시
								작성한 이메일</label>
						</div>
						<div class="form-floating my-1">
							<input type="text" class="form-control" id="inputName"
								placeholder="name" /> <label for="floatingPassword">가입시
								작성한 이름</label>
						</div>
						<div id="find_id" class="w-100 btn btn-lg btn-primary">아이디
							검색</div>
					</div>
				</div>
				...
				<div class="tab-pane fade" id="list-order">
					<div class="d-flex flex-column">
						...
						<div class="input-group my-1">
							<input id="inputEmail2" required="required"
								aria-describedby="button-addon2" autocomplete="off"
								class="form-control " placeholder="이메일을 입력해 주세요" />
							<button id="code-submit" class="btn btn-outline-primary">인증코드
								전송</button>
						</div>
						...
						<div class="d-flex justify-content-between mb-5">
							...
							<div class="input-group w-75">
								<input id="inputCode" required="required"
									aria-describedby="button-addon2" autocomplete="off"
									class="form-control" placeholder="인증번호를 입력해 주세요" disabled />
								<button id="code-check" class="btn btn-outline-primary"
									type="button" disabled="disabled">인증번호 확인</button>
							</div>
						</div>

						<button id="find_pw" class="w-100 btn btn-outline-secondary mb-1"
							disabled="disabled">비밀번호 변경하기</button>
					</div>
				</div>
			</div>
		</section>
		<div id="result" class="col-5 mt-3"></div>
...
/* 	let loading = function(command){
    ...
    let loading = function(command){
		if(!$('#icon-box').length){
			$('body').append(' <div id="icon-box" style="display:none;"></div>');
			$('#icon-box').append('<img src="/images/spinner.gif">');
            $('#icon-box img').css({
                'position':'absolute',
                'left':'50%',
                'top':'50%',

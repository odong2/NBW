...
<script type="text/javascript">
    $(function(){
        ...
        /* 수정 유효성 검사 통과 유무 변수 */
        let mailCheck = true;            // 이메일
        let mailCodeCheck = true;          // 이메일 인증 코드 일치
        let addressCheck = true;            // 주소
        ...
        $("#btn-update").click(function(e){
            ...
            console.log("mailCodeCheck ===> "+ mailCodeCheck);
            ...
            if(mailCheck == false || mailCodeCheck == false || addressCheck == false || nicknameCheck == false || nickEqual ==false ){
                /* 자동 새로고침 방지 */
                e.preventDefault();
                alert("양식을 다시 한번 확인해 주세요.");
                return;
            }else{
                let result = confirm("입력한 정보로 수정하시겠습니까?");
                if(result){
                    alert("수정이 완료되었습니다. 다시 로그인 해주세요.")
                    $("#updateForm").attr("action", "/mypage/modify");
        ...
        $("#btn-cancel").click(function(e){
            let cancel = confirm("수정을 취소하시겠습니까?");
            if(cancel){
                location.href="/home";
            }else{
                e.preventDefault();
        ...
        $("#upBtn-nickname").click(function(){
            ...
            // 취소 버튼을 보여준다.
            $("#backBtn-nick").attr("hidden", false);
            // 닉네임 입력 칸이 나타난다.
            $("#mem_nickname").val("");
        ...
        /*===========  닉네임 변경 취소 =============*/
        $("#backBtn-nick").click(function(){
            let cancel = confirm("메일 변경을 취소하시겠습니까?");
            nicknameCheck = true;
            nickEqual = true;
            if(cancel){
                $("#backBtn-nick").attr("hidden", true);
                $("#mem_nickname").attr("hidden", true);
                $("#lb-nickname").attr("hidden", false);
                $("#upBtn-nickname").attr("hidden", false);
                $("#nicknameCheckDiv").empty();
            }
        })
        ...
        $('#mem_nickname').blur(function (){
            ...
            if(nickname.length < 2){
                ...
                $('#nicknameCheckDiv').text("닉네임은 2글자 이상 입력하셔야 합니다.");
            ...
            $("#nicknameCheckDiv").load("/member/nicknameCheck?nickname="+nickname, function(result) {
                ...
                if(result.indexOf("중복")) {
                    return result;
                    nickEqual = false;
                    console.log("nickEqual ==> "+ nickEqual);
                    // $('#nicknameCheckDiv').remove("alert-success");
                    // $('#nicknameCheckDiv').addClass("alert-danger");
                    // $('#nicknameCheckDiv').text("중복된 닉네임입니다. ");
                    // 중복이 된 경우 가입을 막는다.
                }else {
                    nickEqual=true;
                    console.log("nickEqual ==> "+ nickEqual);
        ...
        $("#upBtn-mail").click(function(){
            ...
            // 취소 버튼 나타난다.
            $("#backBtn-mail").attr("hidden", false);
        ...
        /* 메일 변경 취소 */
        $("#backBtn-mail").click(function(){
            let cancel = confirm("메일 변경을 취소하시겠습니까?");
            mailCodeCheck = true;
            mailCheck = true;
            if(cancel){
                $("#backBtn-mail").attr("hidden", true);
                $("#mem_email").attr("hidden", true);
                $("#btn-email-send").attr("hidden", true);
                $("#lb-mail").attr("hidden", false);
                $("#upBtn-mail").attr("hidden", false);
                $("#mem_email_num").attr("hidden", true);
                $("#message").empty();
            }
        })
        /* ========================== 인증번호 전송 이벤트 ========================== */
            ...
            <div class="row m-4">
                ...
                <div class="col-8 mb-3">
                    <div class="input-group mb-3 w-100">
                        ...
                        <button class="btn-circle" id="backBtn-nick" type="button" style="float: right; border-radius: 10px; border: 0; margin-left: 10px;" hidden>취소</button>
                    </div>
                    <label id="nicknameCheckDiv" class="mb-3 text-danger" style="font-size:0.7rem;"></label>
                </div>
            </div>
            <hr>
            <!-- 생년월일, 성별 정보  -->
            ...
            <div class="row m-4">
                ...
                <div class="col-8 mb-3">
                    <div class="input-group mb-3 w-100">
                        ...
                        <button class="btn-circle" id="backBtn-mail" type="button" style="float: right; border-radius: 10px; border: 0; margin-left: 10px;" hidden>취소</button>
